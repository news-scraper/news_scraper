...
class ArticleScrapePatternsTest < Minitest::Test
  ...
  def setup
    @scrape_patterns = YAML.load_file(NewsScraper.configuration.scrape_patterns_filepath)
  ...
  def test_scrape_methods_have_proper_variable_names
    article_scrape_patterns = File.read(NewsScraper.configuration.scrape_patterns_filepath).split("\n").map(&:strip)
    @scrape_patterns['presets'].each_pair do |data_type, presets|
  ...
  def test_all_preset_xpaths_are_valid
    supported_domains = YAML.load_file(NewsScraper.configuration.scrape_patterns_filepath)['domains'].keys
